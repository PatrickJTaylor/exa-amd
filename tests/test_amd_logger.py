...
def check_output(capsys, l, log_level, on_stdout:bool):
    """
    helper for 'test_stdout_stderr'
    """
    log_name = getattr(l, "logger_name")
    msg = f"{log_level} message"
    getattr(l, log_level)(msg)
...
def test_stdout_stderr(capsys):
    ...
    l = ExaAmdLogger(level_name="DEBUG", logger_name=log_name)
    check_output(capsys, l, "debug", on_stdout=True)
    check_output(capsys, l, "info", on_stdout=True)
    check_output(capsys, l, "warning", on_stdout=False)
    check_output(capsys, l, "error", on_stdout=False)
...
def test_filtering_below_level(configured_log_level, lower_level, capsys):
    ...
    """
    l = ExaAmdLogger(level_name=configured_log_level, logger_name="filter_test")
    msg = "should_not_trigger_an_output"
    getattr(l, lower_level)(msg)
    out, err = capsys.readouterr()
